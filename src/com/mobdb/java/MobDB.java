/*
 * MobDB.java Copyright (C) 2011 mobDB, LLC., This code is provided under the mobDB, LLC. License.
...
public class MobDB {
	...
	public synchronized void execute(String appKey, Push push, String bargraph, boolean secure, MobDBResponseListener listener) throws InvalidParameterException{
		...
		try {
			...
			req.put( Push.PUSH, push.getQueryString() );
	...
	public synchronized void execute( String appKey,String adminKey, String[] sql_query, Object[] parameter, String bargraph, boolean secure, MobDBResponseListener listener) throws InvalidParameterException {
		try {
			...
			sql.put(SDKConstants.QUERY, query);
			req.put(SDKConstants.SQL, sql);
	...
	private static void executeRequest(){
		...
		if(!running){
			...
			running = true;
			currentRequest = (MobDBRequest)requestQueue.elementAt(0);
